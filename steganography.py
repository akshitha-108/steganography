...
class IMG_Stegno:
    ...
    # DataFlair- back function to loop back to the main screen
    def back(self, frame):
        frame.destroy()
        self.main(root)
    ...
    def encode_frame1(self, F):
        F.destroy()
        F2 = Frame(root)
        label1 = Label(F2, text='Set Password:')
        label1.config(font=('Times new roman', 25, 'bold'), bg='#e3f4f1')
        label1.grid()

        password_entry = Entry(F2, show='*', font=('Helvetica', 14))
        password_entry.grid(pady=10)

        button_bws = Button(F2, text='Next', command=lambda: self.encode_frame2(F2, password_entry.get()))
    ...
    def decode_frame1(self, F):
        F.destroy()
        d_f2 = Frame(root)
        label1 = Label(d_f2, text='Enter Password:')
        label1.config(font=('Times new roman', 25, 'bold'), bg='#e3f4f1')
        label1.grid()

        password_entry = Entry(d_f2, show='*', font=('Helvetica', 14))
        password_entry.grid(pady=10)

        button_bws = Button(d_f2, text='Next', command=lambda: self.decode_frame2(d_f2, password_entry.get()))
        button_bws.config(font=('Helvetica', 18), bg='#e8c1c7')
        button_bws.grid()
    ...
    # DataFlair- frame for encode page (password setting)
    def encode_frame2(self, e_F2, password):
    ...
    # DataFlair- frame for decode page (password verification)
    def decode_frame2(self, d_F2, password):
    ...
    # DataFlair- function to modify the pixels of the image
    ...
    # DataFlair- function to enter the data pixels in the image
